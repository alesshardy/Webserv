...
void RequestBody::defineBodyDestination()
{
    ...
    if (method == "POST" && !isCgi)
    {
        ...
        if (_fd == -1)
        {
            return (_request->handleError(500, ERROR, "ERROR: Failed to create upload file: " + _tmpFilePath));
            // perror("open failed");
...
void RequestBody::parseContentLength(const std::string &rawData, size_t  &index, size_t contentLength)
{
    ...
    if (bytesToRead > 0)
    {
        if (write(_fd, rawData.data() + index, bytesToRead) == -1)
            return (_request->handleError(500, ERROR, "ERROR: failed to write in tmp file"));
...
void RequestBody::parseChunked(const std::string &rawData, size_t &index)
{
    while (index < rawData.size())
    {
        ...
        if (chunkSize == 0)
        {
            ...
            if (index + 2 <= rawData.size() && rawData.substr(index, 2) == "\r\n")
            {
                ...
                if (index != rawData.size())
                    return (_request->handleError(400, ERROR, "ERROR: Unexpected data after final chunk"));
        ...
        if (_currentSize + chunkSize > _maxBodySize)
            return (_request->handleError(413, ERROR, "ERROR: Chunked body exceeds maximum allowed size"));
        ...
        if (rawData.substr(index + chunkSize, 2) != "\r\n")
            return (_request->handleError(400, ERROR, "ERROR: Invalid chunked data format"));
...
{
    if (_fd == -1)
        _request->handleError(500, ERROR, "ERROR: Temporary file not initialized");
        // throw std::runtime_error("ERROR: Temporary file not initialized");

    std::ifstream tmpFile(_tmpFilePath.c_str(), std::ios::binary);
    if (!tmpFile.is_open())
        _request->handleError(500, ERROR, "ERROR: Failed to open temporary file");
